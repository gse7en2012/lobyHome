    ...
    .controller('activityDetailsController',
        function ($scope, $timeout, $rootScope, $routeParams, apiService, updateWxTitle) {
            ...
            $scope.joinActivity = function (actId) {
                //$http.post('/api/activity_order?user_id=' + $scope.userInfo.userid + '&activity_id=' + actId).success(function (order) {
                //    $http.get('/wechat/unified_order?user_id=' + $scope.userInfo.userid + '&activity_id=' + actId + '&nonce_str=' + $scope.nonceStr + '&timestamp=' + $scope.timestamp).success(function (data) {
                //        var wxPayConfig = {
